...
def safe_traverse(repo_path: str, max_commits=2000):
    try:
        return list(Repository(repo_path, num_workers=4).traverse_commits())[:max_commits]
    except Exception as e:
        raise RuntimeError(f"Git traversal failed: {e}")

def extract_modified_files(commits):
    modified = []
    for c in commits:
...
def analyze_repo(repo_path: str):
    ...
    files = extract_modified_files(commits)

    contributors = analyze_contributors(commits)
    metrics = compute_metrics(commits, contributors, files)
    activity = compute_activity(commits)
    hotspots = compute_hotspots(commits, files)
